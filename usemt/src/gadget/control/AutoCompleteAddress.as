package gadget.control
{
	...
	import gadget.control.CustomEvent;
	...
	[Event (name="select", type="gadget.control.CustomEvent")]
	
	[Event (name="enter", type="mx.events.FlexEvent")]
	
	[Event (name="change", type="spark.events.TextOperationEvent")]

	public class AutoCompleteAddress extends SkinnableComponent
	{
		...
		public function AutoCompleteAddress()
		{
			...
			this.setStyle("skinClass", Class(AutoCompleteAddressSkin));
...
//				inputTxt.addEventListener(FlexEvent.ENTER, enter);
			...
			if (instance==list){
				...
				list.addEventListener(MouseEvent.MOUSE_OVER, showToolTipListener);
			}
...
//				dropDown.addEventListener(MouseEvent.MOUSE_OVER, showToolTipListener);
			}
		}
		...
		private function showToolTipListener(event:MouseEvent):void {
			if(event.relatedObject is DefaultItemRenderer){
				var curTarget:Object = (event.relatedObject as DefaultItemRenderer).data;
				if(curTarget != null) {
					var lblTooltip:String = curTarget.data ;
					
					if("AccountName"==curTarget.fieldName){
						lblTooltip = lblTooltip + ", " 
							+ curTarget.PrimaryBillToStreetAddress + " " 
							+ curTarget.PrimaryBillToCity + " " 
							+ curTarget.PrimaryBillToCountry;
					}else if("PrimaryShipToStreetAddress"==curTarget.fieldName){
						lblTooltip = lblTooltip + ", " 
							+ curTarget.PrimaryShipToStreetAddress + " " 
							+ curTarget.PrimaryShipToCity + " " 
							+ curTarget.PrimaryShipToCountry;
					}
//					list.toolTip = lblTooltip;
					(event.relatedObject as DefaultItemRenderer).toolTip = lblTooltip;
				}
			}
		}
