package com.adobe.protocols.oauth2
{
	...
	public class OAuth2 extends EventDispatcher
	{
		...
		private function getAccessTokenWithAuthorizationCodeGrant(authorizationCodeGrant:AuthorizationCodeGrant):void
		{
			...
			// add event listeners
			authorizationCodeGrant.stageWebView.addEventListener(HTTPStatusEvent.HTTP_RESPONSE_STATUS, onLocationChanging);
			...
			loadURL(authorizationCodeGrant.stageWebView,authorizationCodeGrant.getFullAuthUrl(authEndpoint));
			
			function onLocationChanging(locationChangeEvent:HTTPStatusEvent):void
			{
				var location:String = getHeaderValue(locationChangeEvent.responseHeaders,'Location');
				log.info("Loading URL: " + location);
				
				if (location!=null && location!='' && location.indexOf(authorizationCodeGrant.redirectUri) == 0 && location.indexOf(OAuth2Const.RESPONSE_PROPERTY_AUTHORIZATION_CODE) > 0)
				{
		...
		private function loadURL(loader:URLLoader,url:String,followRedirects:Boolean = true):void{
		...
		private function getAccessTokenWithImplicitGrant(implicitGrant:ImplicitGrant):void
		{
			...
			// add event listeners
			implicitGrant.stageWebView.addEventListener(HTTPStatusEvent.HTTP_RESPONSE_STATUS, onLocationChange);
			...
			loadURL(implicitGrant.stageWebView,implicitGrant.getFullAuthUrl(authEndpoint));
			
			function onLocationChange(locationChangeEvent:HTTPStatusEvent):void
			{
				var location:String = getHeaderValue(locationChangeEvent.responseHeaders,'Location');
				log.info("Loading URL: " + location);
				if (location!=null && location.indexOf(implicitGrant.redirectUri) == 0 && location.indexOf(OAuth2Const.RESPONSE_PROPERTY_ACCESS_TOKEN) > 0)
				{
